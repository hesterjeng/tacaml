...
(** Convert a Safe.t indicator to a list of Indicator.t outputs.
    ...
    - [safe_to_indicators (S.Macd { fast_period = 12; slow_period = 26; signal_period = 9 })] returns
      [[F (Indicator.Float.Macd_MACD { ... }); F (Indicator.Float.Macd_MACDSignal { ... }); F (Indicator.Float.Macd_MACDHist { ... })]]
*)
...
(** Convert an Indicator.t back to its corresponding Safe.t.
    ...
    since all Indicator.t values are derived from Safe.t indicators.
    Multiple Indicator.t values from the same Safe.t (like MACD components)
    will all map back to the same Safe.t.
    
    Example:
    - [indicator_to_safe (F (Indicator.Float.Sma { timeperiod = 14 }))] returns
      [Pack.pack (S.Sma { timeperiod = 14 })]
    - [indicator_to_safe (F (Indicator.Float.Macd_MACD { fast_period = 12; slow_period = 26; signal_period = 9 }))] returns
