...
let pp : type a b. Format.formatter -> (a, b) t -> unit =
 fun fmt x ->
  ...
  let name, params =
    match x with
    | Accbands { timeperiod } ->
      ("Accbands", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Acos _ -> ("Acos", [])
    | Ad _ -> ("Ad", [])
    | Add _ -> ("Add", [])
    ...
    | Adxr { timeperiod } ->
      ("Adxr", [ "timeperiod=" ^ string_of_int timeperiod ])
    ...
    | Aroon { timeperiod } ->
      ("Aroon", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Aroonosc { timeperiod } ->
      ("Aroonosc", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Asin _ -> ("Asin", [])
    | Atan _ -> ("Atan", [])
    | Atr { timeperiod } -> ("Atr", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Avgprice _ -> ("Avgprice", [])
    | Avgdev { timeperiod } ->
      ("Avgdev", [ "timeperiod=" ^ string_of_int timeperiod ])
    ...
    | Beta { timeperiod } ->
      ("Beta", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Bop _ -> ("Bop", [])
    ...
    | Cdlabandonedbaby { penetration } ->
      ("Cdlabandonedbaby", [ "penetration=" ^ string_of_float penetration ])
    | Cdladvanceblock _ -> ("Cdladvanceblock", [])
    | Cdlbelthold _ -> ("Cdlbelthold", [])
    | Cdlbreakaway _ -> ("Cdlbreakaway", [])
    | Cdlclosingmarubozu _ -> ("Cdlclosingmarubozu", [])
    ...
    | Cdldarkcloudcover { penetration } ->
      ("Cdldarkcloudcover", [ "penetration=" ^ string_of_float penetration ])
    | Cdldoji _ -> ("Cdldoji", [])
    | Cdldojistar _ -> ("Cdldojistar", [])
    | Cdldragonflydoji _ -> ("Cdldragonflydoji", [])
    | Cdlengulfing _ -> ("Cdlengulfing", [])
    | Cdleveningdojistar { penetration } ->
      ("Cdleveningdojistar", [ "penetration=" ^ string_of_float penetration ])
    | Cdleveningstar { penetration } ->
      ("Cdleveningstar", [ "penetration=" ^ string_of_float penetration ])
    | Cdlgapsidesidewhite _ -> ("Cdlgapsidesidewhite", [])
    | Cdlgravestonedoji _ -> ("Cdlgravestonedoji", [])
    | Cdlhammer _ -> ("Cdlhammer", [])
    | Cdlhangingman _ -> ("Cdlhangingman", [])
    ...
    | Cdlmathold { penetration } ->
      ("Cdlmathold", [ "penetration=" ^ string_of_float penetration ])
    | Cdlmorningdojistar { penetration } ->
      ("Cdlmorningdojistar", [ "penetration=" ^ string_of_float penetration ])
    | Cdlmorningstar { penetration } ->
      ("Cdlmorningstar", [ "penetration=" ^ string_of_float penetration ])
    | Cdlonneck _ -> ("Cdlonneck", [])
    | Cdlpiercing _ -> ("Cdlpiercing", [])
    | Cdlrickshawman _ -> ("Cdlrickshawman", [])
    | Cdlrisefall3methods _ -> ("Cdlrisefall3methods", [])
    ...
    | Correl { timeperiod } ->
      ("Correl", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Cos _ -> ("Cos", [])
    | Cosh _ -> ("Cosh", [])
    | Dema { timeperiod } ->
      ("Dema", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Div _ -> ("Div", [])
    ...
    | Kama { timeperiod } ->
      ("Kama", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Linearreg { timeperiod } ->
      ("Linearreg", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Linearreg_angle { timeperiod } ->
      ("Linearreg_angle", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Linearreg_intercept { timeperiod } ->
      ("Linearreg_intercept", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Linearreg_slope { timeperiod } ->
      ("Linearreg_slope", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Ln _ -> ("Ln", [])
    | Log10 _ -> ("Log10", [])
    ...
    | Macdfix { signal_period } ->
      ("Macdfix", [ "signal_period=" ^ string_of_int signal_period ])
    ...
    | Maxindex { timeperiod } ->
      ("Maxindex", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Medprice _ -> ("Medprice", [])
    | Mfi { timeperiod } -> ("Mfi", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Midpoint { timeperiod } ->
      ("Midpoint", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Midprice { timeperiod } ->
      ("Midprice", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Min { timeperiod } -> ("Min", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Minindex { timeperiod } ->
      ("Minindex", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Minmax { timeperiod } ->
      ("Minmax", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Minmaxindex { timeperiod } ->
      ("Minmaxindex", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Minus_di { timeperiod } ->
      ("Minus_di", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Minus_dm { timeperiod } ->
      ("Minus_dm", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Mom { timeperiod } -> ("Mom", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Mult _ -> ("Mult", [])
    | Natr { timeperiod } ->
      ("Natr", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Obv _ -> ("Obv", [])
    | Plus_di { timeperiod } ->
      ("Plus_di", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Plus_dm { timeperiod } ->
      ("Plus_dm", [ "timeperiod=" ^ string_of_int timeperiod ])
    ...
    | Rocp { timeperiod } ->
      ("Rocp", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Rocr { timeperiod } ->
      ("Rocr", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Rocr100 { timeperiod } ->
      ("Rocr100", [ "timeperiod=" ^ string_of_int timeperiod ])
    ...
    | Tanh _ -> ("Tanh", [])
    | Tema { timeperiod } ->
      ("Tema", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Trange _ -> ("Trange", [])
    | Trima { timeperiod } ->
      ("Trima", [ "timeperiod=" ^ string_of_int timeperiod ])
    | Trix { timeperiod } ->
      ("Trix", [ "timeperiod=" ^ string_of_int timeperiod ])
    ...
    | Wclprice _ -> ("Wclprice", [])
    | Willr { timeperiod } ->
      ("Willr", [ "timeperiod=" ^ string_of_int timeperiod ])
