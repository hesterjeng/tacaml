...
type ('a, 'b) indicator_params =
  ...
  | Aroon : { timeperiod : int } -> (Ohlcv.t, float_ba * float_ba) indicator_params
  ...
  | Beta : { timeperiod : int } -> (float_ba * float_ba, float_ba) indicator_params
  ...
  | Cdlabandonedbaby : { penetration : float } -> (Ohlcv.t, int_ba) indicator_params
  ...
  | Cdldarkcloudcover : { penetration : float } -> (Ohlcv.t, int_ba) indicator_params
  ...
  | Cdleveningdojistar : { penetration : float } -> (Ohlcv.t, int_ba) indicator_params
  | Cdleveningstar : { penetration : float } -> (Ohlcv.t, int_ba) indicator_params
  ...
  | Cdlmorningdojistar : { penetration : float } -> (Ohlcv.t, int_ba) indicator_params
  | Cdlmorningstar : { penetration : float } -> (Ohlcv.t, int_ba) indicator_params
  ...
  | Correl : { timeperiod : int } -> (float_ba * float_ba, float_ba) indicator_params
  ...
  | Linearreg_angle : { timeperiod : int } -> (float_ba, float_ba) indicator_params
  | Linearreg_intercept : { timeperiod : int } -> (float_ba, float_ba) indicator_params
  | Linearreg_slope : { timeperiod : int } -> (float_ba, float_ba) indicator_params
  | Ln : unit -> (float_ba, float_ba) indicator_params
  | Log10 : unit -> (float_ba, float_ba) indicator_params
  | Ma : { timeperiod : int; ma_type : Ma_type.t } -> (float_ba, float_ba) indicator_params
  | Macd : {
      fast_period : int;
      slow_period : int;
      signal_period : int;
  ...
  | Minmax : { timeperiod : int } -> (float_ba, float_ba * float_ba) indicator_params
  | Minmaxindex : { timeperiod : int } -> (float_ba, int_ba * int_ba) indicator_params
  ...
  | Sar : { acceleration : float; maximum : float } -> (Ohlcv.t, float_ba) indicator_params
  | Sarext : {
      start_value : float;
  ...
  | Stddev : { timeperiod : int; nb_dev : float } -> (float_ba, float_ba) indicator_params
  | Stoch : {
      fast_k_period : int;
      slow_k_period : int;
  ...
  | T3 : { timeperiod : int; v_factor : float } -> (float_ba, float_ba) indicator_params
  ...
  | Ultosc : {
      ...
      -> (Ohlcv.t, float_ba) indicator_params
  | Var : { timeperiod : int; nb_dev : float } -> (float_ba, float_ba) indicator_params
...
let calculate : type a b.
 ...
 fun source params output ->
  let startIdx = 0 in
  let offset = lookback params in
  let slice arr =
    let len = Bigarray.Array1.dim arr in
    if offset >= len then
      Bigarray.Array1.create (Bigarray.Array1.kind arr) (Bigarray.Array1.layout arr) 0
    else
      Bigarray.Array1.sub arr offset (len - offset)
  in
  match (params, source, output) with
  | Accbands { timeperiod }, ohlcv, (out1, out2, out3) ->
    let endIdx = Bigarray.Array1.dim ohlcv.high - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_accbands startIdx endIdx (ba ohlcv.high) (ba ohlcv.low)
          (ba ohlcv.close) timeperiod outBegIdx outNBElement (ba (slice out1))
          (ba (slice out2)) (ba (slice out3)))
  | Acos (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_acos startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Ad (), ohlcv, out ->
    let endIdx = Bigarray.Array1.dim ohlcv.high - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_ad startIdx endIdx (ba ohlcv.high) (ba ohlcv.low) (ba ohlcv.close)
          (ba ohlcv.volume) outBegIdx outNBElement (ba (slice out)))
  | Add (), (f1, f2), out ->
    let endIdx = Bigarray.Array1.dim f1 - 1 in
    wrap (fun outBegIdx outNBElement ->
  ...
  | Asin (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_asin startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Atan (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_atan startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  ...
  | Bop (), ohlcv, out ->
    ...
    wrap (fun outBegIdx outNBElement ->
        F.ta_bop startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high) (ba ohlcv.low)
          (ba ohlcv.close) outBegIdx outNBElement (ba (slice out)))
  ...
  | Ceil (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_ceil startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  ...
  | Cos (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_cos startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Cosh (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_cosh startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  ...
  | Exp (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_exp startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Floor (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_floor startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Ht_dcperiod (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
  ...
  | Log10 (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_log10 startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Ma { timeperiod; ma_type }, f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_ma startIdx endIdx (ba f) timeperiod
          (Ma_type.to_int ma_type)
          outBegIdx outNBElement (ba (slice out)))
  ...
  | Mfi { timeperiod }, ohlcv, out ->
    ...
    wrap (fun outBegIdx outNBElement ->
        F.ta_mfi startIdx endIdx (ba ohlcv.high) (ba ohlcv.low) (ba ohlcv.close)
          (ba ohlcv.volume) timeperiod outBegIdx outNBElement (ba (slice out)))
  ...
  | Minmaxindex { timeperiod }, f, (out1, out2) ->
    ...
    wrap (fun outBegIdx outNBElement ->
        F.ta_minmaxindex startIdx endIdx (ba f) timeperiod outBegIdx
          outNBElement (iba (slice out1)) (iba (slice out2)))
  ...
  | Sar { acceleration; maximum }, ohlcv, out ->
    ...
    wrap (fun outBegIdx outNBElement ->
        F.ta_sar startIdx endIdx (ba ohlcv.high) (ba ohlcv.low) acceleration
          maximum outBegIdx outNBElement (ba (slice out)))
  | ( Sarext
        {
          start_value;
          offset_on_reverse;
    ...
    wrap (fun outBegIdx outNBElement ->
        F.ta_sarext startIdx endIdx (ba ohlcv.high) (ba ohlcv.low) start_value
          ...
          acceleration_max_short outBegIdx outNBElement (ba (slice out)))
  | Sin (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_sin startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Sinh (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_sinh startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  ...
  | Sqrt (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_sqrt startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  ...
  | Tan (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_tan startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
  | Tanh (), f, out ->
    let endIdx = Bigarray.Array1.dim f - 1 in
    wrap (fun outBegIdx outNBElement ->
        F.ta_tanh startIdx endIdx (ba f) outBegIdx outNBElement (ba (slice out)))
