...
let ta_accbands startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outRealUpperBand
...
let ta_ad startIdx endIdx (ohlcv : ohlcv) outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_adx startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
      F.ta_adx startIdx endIdx (ba ohlcv.high) (ba ohlcv.low) (ba ohlcv.close)
        optInTimePeriod outBegIdx outNBElement (ba outReal))

let ta_adxr startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_aroon startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outAroonDown
    outAroonUp =
  wrap (fun outBegIdx outNBElement ->
...
let ta_aroonosc startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_avgprice startIdx endIdx (ohlcv : ohlcv) outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_bbands startIdx endIdx inReal optInTimePeriod optInNbDevUp optInNbDevDn
    optInMAType outRealUpperBand outRealMiddleBand outRealLowerBand =
  wrap (fun outBegIdx outNBElement ->
...
let ta_cdl3blackcrows startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdl3blackcrows startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdl3inside startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdl3inside startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdl3linestrike startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdl3linestrike startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdl3outside startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdl3outside startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdl3starsinsouth startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdl3starsinsouth startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdl3whitesoldiers startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
...
let ta_cdladvanceblock startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdladvanceblock startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlbelthold startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlbelthold startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlbreakaway startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlbreakaway startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlclosingmarubozu startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlclosingmarubozu startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlconcealbabyswall startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlconcealbabyswall startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlcounterattack startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
...
let ta_cdldoji startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdldoji startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdldojistar startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdldojistar startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdldragonflydoji startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdldragonflydoji startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlengulfing startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
...
let ta_cdleveningstar startIdx endIdx (ohlcv : ohlcv) optInPenetration
    outInteger =
  wrap (fun outBegIdx outNBElement ->
...
let ta_cdlmorningstar startIdx endIdx (ohlcv : ohlcv) optInPenetration
    outInteger =
  wrap (fun outBegIdx outNBElement ->
...
let ta_cdlonneck startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlonneck startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlpiercing startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlpiercing startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlrickshawman startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlrickshawman startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlrisefall3methods startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlrisefall3methods startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlseparatinglines startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlseparatinglines startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlshootingstar startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlshootingstar startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlshortline startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlshortline startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlspinningtop startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlspinningtop startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlstalledpattern startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlstalledpattern startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlsticksandwich startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlsticksandwich startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdltakuri startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdltakuri startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdltasukigap startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdltasukigap startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlthrusting startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlthrusting startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdltristar startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdltristar startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlunique3river startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlunique3river startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlupsidegap2crows startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
      F.ta_cdlupsidegap2crows startIdx endIdx (ba ohlcv.open_) (ba ohlcv.high)
        (ba ohlcv.low) (ba ohlcv.close) outBegIdx outNBElement (iba outInteger))

let ta_cdlxsidegap3methods startIdx endIdx (ohlcv : ohlcv) outInteger =
  wrap (fun outBegIdx outNBElement ->
...
let ta_dx startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_imi startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_ma startIdx endIdx inReal optInTimePeriod optInMAType outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_macdext startIdx endIdx inReal optInFastPeriod optInFastMAType
    optInSlowPeriod optInSlowMAType optInSignalPeriod optInSignalMAType outMACD
...
let ta_mama startIdx endIdx inReal optInFastLimit optInSlowLimit outMAMA outFAMA
    =
  wrap (fun outBegIdx outNBElement ->
...
let ta_medprice startIdx endIdx (ohlcv : ohlcv) outReal =
  wrap (fun outBegIdx outNBElement ->
      F.ta_medprice startIdx endIdx (ba ohlcv.high) (ba ohlcv.low) outBegIdx
        outNBElement (ba outReal))

let ta_mfi startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_midprice startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_minus_di startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
      F.ta_minus_di startIdx endIdx (ba ohlcv.high) (ba ohlcv.low)
        (ba ohlcv.close) optInTimePeriod outBegIdx outNBElement (ba outReal))

let ta_minus_dm startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_natr startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_plus_di startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
      F.ta_plus_di startIdx endIdx (ba ohlcv.high) (ba ohlcv.low)
        (ba ohlcv.close) optInTimePeriod outBegIdx outNBElement (ba outReal))

let ta_plus_dm startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_stoch startIdx endIdx (ohlcv : ohlcv) optInFastK_Period optInSlowK_Period
    optInSlowK_MAType optInSlowD_Period optInSlowD_MAType outSlowK outSlowD =
  wrap (fun outBegIdx outNBElement ->
...
let ta_stochf startIdx endIdx (ohlcv : ohlcv) optInFastK_Period
    optInFastD_Period optInFastD_MAType outFastK outFastD =
  wrap (fun outBegIdx outNBElement ->
...
let ta_trange startIdx endIdx (ohlcv : ohlcv) outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_typprice startIdx endIdx (ohlcv : ohlcv) outReal =
  wrap (fun outBegIdx outNBElement ->
...
let ta_wclprice startIdx endIdx (ohlcv : ohlcv) outReal =
  wrap (fun outBegIdx outNBElement ->
      F.ta_wclprice startIdx endIdx (ba ohlcv.high) (ba ohlcv.low)
        (ba ohlcv.close) outBegIdx outNBElement (ba outReal))

let ta_willr startIdx endIdx (ohlcv : ohlcv) optInTimePeriod outReal =
  wrap (fun outBegIdx outNBElement ->
